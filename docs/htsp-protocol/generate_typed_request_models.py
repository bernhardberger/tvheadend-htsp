...
OUTPUT = (
    ...
    / "sdk/htsp-protocol/src/main/kotlin/at/bernhardberger/tvheadend/htsp/GeneratedHtspRequests.kt"
)
...
def render() -> str:
    lines = [
        ...
        "package at.bernhardberger.tvheadend.htsp", "", "import java.util.Collections", "",
    ]
    api_response = next(value for value in REQUEST_PUBLIC_DECLARATIONS if value.name == "ApiResponse")
    for declaration in REQUEST_PUBLIC_DECLARATIONS:
        if declaration is api_response:
            continue
        lines.extend(render_declaration(declaration))
    lines.extend(render_request(ENTRIES["api"], MODELS["api"]))
    lines.extend(render_declaration(api_response))
    for method in ("hello", "authenticate"):
        lines.extend(render_request(ENTRIES[method], MODELS[method]))
    for entry in CATALOG[:36]:
