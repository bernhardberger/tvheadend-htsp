...
The current immutable release coordinate is `at.bernhardberger.tvheadend:htsp:0.6.0`.
`0.6.0` is a provisional major-zero release with intentional Kotlin source,
JVM binary, and behavioral changes from `0.5.0`. Published release bytes are
immutable and must never be replaced.

The `0.6.0` coordinate is available from
[Maven Central](https://central.sonatype.com/artifact/at.bernhardberger.tvheadend/htsp/0.6.0),
with its [repository files](https://repo1.maven.org/maven2/at/bernhardberger/tvheadend/htsp/0.6.0/)
...
Read the `0.6.0` release notes before using it as a baseline. Local checks and
